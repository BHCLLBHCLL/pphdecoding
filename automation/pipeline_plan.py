...
# BAM 前置网格器/选择/流体域配置（box_scflow_mdl.vbs :71-348 录制锁定；
# SetUseAFFacetter=True 等 AF faceter 配置为 FindAFFaceMatching 前提，
# 缺失时 FindAFFaceMatching 空引用 AV（P12-A 实测）。getter 行由
# to_vbs_actions 自动注入，模板仅含 Param/动作行）。
MESHER_SETTINGS_ACTIONS: list[str] = [
    'Param1_ = "poly"',
    'Param2_ = False',
...
LOCKED_COMMANDS: dict[str, str] = {
    ...
    "mesher_settings": "\n".join(MESHER_SETTINGS_ACTIONS),          # :71-348 (mdl)
