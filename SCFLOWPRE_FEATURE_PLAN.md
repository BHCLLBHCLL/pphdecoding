...
- ✅ BAM 录制锁定（`box_scflow_mdl.vbs`）：`BeginMDLWizard → CreateBoundary →
  ...
  → RepairMDL → EndMDLWizard`
- ✅ Wrapping 录制锁定（`box_scflow_wrapping.vbs`）：`Doc_.BeginWrapping →
  CreateWrappingGroup → WrappingGroup_.GetOctParam/GetWrappingParam →
  CreateOctree → ExecuteWrapping ×4 → Octree_.UpdateGroups → ExecuteWrapping
  ×2 → EndWrapping`；Execute 面板新增 “Wrapping (from CAD)” 步骤，
  未勾选 API 时原生写出 `meshinggroup1_wrap.mdl`
