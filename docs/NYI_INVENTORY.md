# PPH Viewer NYI 菜单清单

> 由 `tools/scan_nyi_menus.py` 自动生成。
> 对应日志：`[…] not available in PPH viewer`（现已灰显）。

合计 **34** 项。

## File(&F)

- Create Actran Files…
## Edit(&E)

- Define Facet Part…
- Create Non-Facet/Closed Volume Part…
- Create 2D Sub-mesh Meshing Unit…
## Ridge

- Refine Octants
- Refine Octants (Recursive)…
- Refine Octants from Curvature…
- Refine Octants from Separation…
- Merge Octants
- Show Octants by Marked Face
- Show Octants by Marked Edge
- Restore Closed Volume Data…
- Fix Marked Element Shape
## Select(&S)

- Spread Selected Face to Selected Edge
- Select by Element Number…
- Select Elements by List File…
- Select Faces That Have the Same Area
- Element Quality Check…
- Check Intersection
## View(&V)

- Change Display Type of Edge
- Switch Display Surface by Orientation
## Rubber Circle

- Rubber Circle (Show)
- Rubber Circle (Hide)
## Rubber Polygon

- Rubber Polygon (Show)
- Rubber Polygon (Hide)
- Refinement Level…
- Display Octants Connected by Node
- Display Octants Connected by Face
- Display Neighbor Octants by Direction…
- Report Prism Layer
- Element Types…
- Show Parts List Dialog…
- Show Region Registration Check Dialog…
- Element Quality Check…
