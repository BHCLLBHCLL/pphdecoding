...
* 契约门 8 项全绿，但**门自己**没有覆盖测试：`test_r53_evidence.py` 只测了第 8 项，
  前 7 项靠各自的轮次测试分散覆盖 —— 门"整体是否还能挡住"没有一条总账；
* `docs/NYI_INVENTORY.md` 现在四段同源，但**缺语料分组**只列了组名与类名，没写
  "补了这个语料之后能多覆盖几类"（排期时看不出优先级）；
* 复验窗口检查（`sweep_reopen_check`）是**手动跑**的，没有"定期提醒"的落点。

### 条目

| # | 条目 | 做法 | 验收句 | 人日 |
|---|---|---|---|---|
| **R54-1** | **门自己也要有总账** | 给 8 项各写"合成反例"（每项至少一个假证据形态必须 FAIL） | 8 项各有反例且全 FAIL + 测试 | 0.5 |
| **R54-2** | **缺语料优先级** | 缺语料分组补"补上后预计能多覆盖几类"（按组内类的取法可达性估算） | 分组带收益估计 + 测试 | 0.25 |
| **R54-3** | **复验提醒落点** | 把 `sweep_reopen_check` 接进 `run_all_tests.py`（门之后跑，异常才出声） | 一条命令可见 + 测试 | 0.25 |
