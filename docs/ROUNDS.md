...
一起收进了仓库，R49 又误收 3 个（`_r49_*.py`）—— 根因是 `tools/git_milestone.py` 的收录
模式含根目录 `*.py`，而草稿脚本正好落在根目录。

* R48 删掉 R47 误收的 9 个；**R50 删掉 R49 误收的 3 个**；
* **根因也在 R50 修掉**：`_is_excluded()` 现在排除**根目录的 `_*.py`**（草稿脚本约定
  以 `_` 开头；`tools/_p12*.py` 与 `_p12*/*.json` 证据不受影响）—— 下次不用靠自觉了。

顺带说明为什么 `api_contract_check` 的"仓内引用"检查会被草稿脚本误伤（R45 也踩过同一个坑）。
...
## R50 —— 边界样本量误杀 + 证据可复验串 + 普查收口声明（2026-09-15，✅ 已完成）

### 条目与结果

| # | 条目 | 验收句 | 结果 |
|---|---|---|---|
| **R50-1** | **边界样本量误杀** | 误杀/误放各有数 + 阈值建议 | ✅ 自类通过 **108/108**（`false_kill 0`）+ 跨类 **0/120** + 近阈否样本 **0/6** + 边界表；建议**维持半数** |
| **R50-2** | **证据可复验串** | 任取一条能追到某轮某日志 | ✅ `--evidence-run` → 目录条目 `evidence_run`（27 条 `host_absent` + 4 类 `recipe_unreliable` 逐条带） |
| **R50-3** | **普查收口声明** | 文档一处宣告 + 测试锁住 | ✅ 审计 §64.3 三条判据；`test_r50_evidence.py` 把下限钉在 **155 类** |

### R50-1 判据的另一面

R49 只量了误放。本轮把**误杀**这一面补齐 —— 三条证据（全部落在证据文件里）：

| 指标 | 值 | 说明 |
|---|---|---|
| 自类通过 | **108/108**（`false_kill = 0`） | 覆盖**全部**已取得类（R49 只抽样 40） |
| 跨类误放 | **0/120（0.0%）** | 抽样 40 类 × 3 别家 |
| 近阈否样本 | **0 / 6**（`|2r − s| ≤ 1`） | 被否的都在阈值以下很远 → 不是脆判决 |
| 边界表 | `2r ≥ s` 放行 / `2r = s−1` 拒绝 | 样本 1..5 × 解析 0..s，离线确定性 |

**阈值建议：维持"半数"**。改宽会放进别家对象（0/120 是靠它），改严会误杀独有成员少的类
（现在 108/108 全过）。

### R50-2 可复验串

`R50 | 2026-09-23T20:52:01 | log=_p12u_gate\r50\name_verdicts.json | projects=box.pph,exB01-1_intake_manifold.pph,exA26-1_ldc.pph,exA16-2.pph,exA25-1.pph,exA18-4.pph`

写进证据（`evidence_run` 顶层 + coverage）并在提取期灌进目录：`host_absent` 27 条、
`recipe_unreliable` 4 类**逐条**带。测试断言"每一条都有、且含轮次与 `log=`"。

### R50-3 收口声明（三条判据同时成立）

1. 剩余类**全部有终态**：29/29（六种终态，零"未归因"）；
2. **边际收益 < 1 类/轮**趋势已现：R47 +3 → R48 +3 → R49 +3 → **R50 +0**（本轮不做扩面）；
   剩下 25/29 是 `needs-corpus` —— 缺的是**语料**不是机制；
3. **覆盖率锁死 155 类**（`test_r50_evidence.py`，只许升不许降）。

**复验窗口**（任一出现即重开）：① 新增语料（材料库/CoSim/粒子/映射算例）；
② 宿主版本变化；③ 出现"手册声明了、既拿不到也解释不了"的新证据。

### 回归

全量回归 **1549 passed / 4 skipped / 0 failed**（571.61 s，带 `-rs` 落盘 skip 原因：
`test_material_prp_write` 1 + `test_native_bridge` 3，全是稳定环境项；R49 收口同口径 1536/4/0，
增量 +13 = 本轮新模块 13 项）。新增测试 1 个模块 `test_r50_evidence.py`（13 项：
误杀/边界表/追溯串/收口锁）。

### 证据

`schemas/host_member_availability.json`（155 类 / 3925 成员 / 未实现 27 / errors 0 /
`guard_audit`（`false_kill 0` / `boundary` / `rejection_margins`）/
`evidence_run`）、`schemas/unswept_account.json`（29 类终态）、`schemas/vb_api_catalog.json`
（`host_absent` 27 + `evidence_run` 逐条）、`_p12u_gate/r50/name_verdicts.json`。
...
## R53 —— 结论进自动文档 + 盯防清单带动作 + 收口结论进契约门（2026-09-15，✅ 已完成）

收口之后把结论**锁住**：文档自动生成（与产品面同源）、盯防清单可执行、契约门加第 8 项。

### 条目与结果

| # | 条目 | 验收句 | 结果 |
|---|---|---|---|
| **R53-1** | **结论进自动文档** | 文档一处可读 + 测试对账 | ✅ `docs/NYI_INVENTORY.md` 那一节改为**逐行渲染** `render_capability_report()`（四段） |
| **R53-2** | **盯防清单带动作** | 每条有可执行动作 + 测试 | ✅ `--watchlist` 每条带 `actions`（试哪个取法 / 看哪条证据） |
| **R53-3** | **收口结论进契约门** | 门 8/8 PASS 且能挡住回落 | ✅ 第 8 项 `check_sweep_convergence()`（三条判据）+ 合成证据验"挡得住" |

### R53-1 文档与产品面同源（逐行对账）

`tools/scan_nyi_menus.py` 的「宿主侧能力边界」节不再自己拼列表，而是**直接调**
`render_capability_report()` 把四段（未实现成员 / 取法不可照抄 / 取不到实例 / 缺语料分组）
一次渲染进文档 —— 测试逐行断言"文档里的块 == 产品面输出"，三面（API/面板/文档）不可能漂移。
> 踩坑：脚本方式运行时仓库根不在 `sys.path`，第一次生成出来是
> `（能力汇总不可用：ModuleNotFoundError）` —— 已在工具里 `sys.path.insert(0, ROOT)`。

### R53-2 每条盯防项都能直接动手

`WATCH_KINDS` 只说"为什么盯"，新增 `_action()` 给"怎么做"（都引证据）：

| 档 | 动作 |
|---|---|
| `swept_suspect` | 重取实例并用**更大的独有成员样本**重验（附上次 x/y 未知） |
| `near_threshold` | 复验取法 `<plan>`：样本 s 解析 r（margin m，差一点翻案） |
| `empty_object` | 先跑 `<hint>`；再试 `<auto_empty_targets 里的取法>` |
| `probe_limitation` | 试 `<手册声明的取法>`；看 `<证据文件>` 里的调用错误 |
| `no_member` | 看手册该页是否补了成员（当前 0 成员） |

### R53-3 契约门第 8 项

`check_sweep_convergence()`：① 覆盖率 ≥ 收口下限（155，"只许升不许降"）；② 未普查类集
与终态表**完全一致**（`unattributed`/`extra_rows` 皆空、且每行有 `terminal` + `reason`）；
③ 复验窗口结论为"无需重开"。路径与下限**可注入** —— 测试拿合成证据跑三种掉线形态
（覆盖率 3 / 多一个类 / 少一行），确认每种都被挡住。门现在是 **8/8 PASS**。

> 顺带清掉 `api_contract_check.py` 里一个死导入（`import subprocess`，文件内未使用）——
> 与 R42 删死代码同一条纪律。

### 回归

全量回归 **1592 passed / 4 skipped / 0 failed**（571.80 s，`-rs` 落盘 skip 原因全为稳定环境项；
R52 收口同口径 1580/4/0，增量 +12 = 本轮新模块 12 项）。新增测试 1 个模块 `test_r53_evidence.py`（12 项：
文档逐行对账 / 动作可执行 / 门第 8 项与三种掉线形态）。

### 证据

`docs/NYI_INVENTORY.md`（四段汇总 + 27 条未实现成员逐条带下一步）、
`tools/api_contract_check.py`（8 项）、`tools/sweep_reopen_check.py`（`--watchlist` 带动作）、
`tools/scan_nyi_menus.py`、`schemas/unswept_account.json`（29 类终态）。

---

## R54 —— 契约门自测总账 + 缺语料收益估计 + 复验提醒落点（2026-09-15，✅ 已完成）
...
| # | 条目 | 验收句 | 结果 |
|---|---|---|---|
| **R54-1** | **门自己也要有总账** | 8 项各有反例且全 FAIL | ✅ `--self-test`：**8/8 挡住**（每项一个合成反例，跑完还原） |
| **R54-2** | **缺语料优先级** | 分组带收益估计 + 测试 | ✅ `needs_corpus_plan`：每组"补上后预计多覆盖几类" |
| **R54-3** | **复验提醒落点** | 一条命令可见 + 测试 | ✅ `run_all_tests.reopen_notice()`（健康时**静默**，建议重开才出声） |

### R54-1 每项门都有反例（自测总账）

`python tools/api_contract_check.py --self-test` 给 8 项各喂一个**合成反例**，必须全部 FAIL：

| 项 | 反例 |
|---|---|
| catalog | 参数名以引号开头（假参数） |
| ledger | 有缺口却没终态 |
| host_absent | 合成仓里 `offender.py` 引用了无歧义的未实现成员 |
...
为此给 4 项加了**路径注入**（catalog/ledger/account/convergence），另 3 项用"临时替换模块级名字"的补丁（`_Patch`，跑完还原 —— 测试专门断言还原）。
> 自测首轮 4 项"放行"，两个原因都记档：① 4 个 lambda 全部捕获同一个变量 `p`（**晚绑定**，
> 都指向最后写的那个文件）；② 三项检查没有注入点。这正是"门自己也要有总账"的价值 ——
> 不然永远以为门是好的。

### R54-2 缺语料的收益估计

`needs_corpus_plan`（`schemas/unswept_account.json`）：判据 = 该类**有已知取法**
（目录/片段给了候选，或手册给了配方 —— 配方的宿主正是"缺的那个前置对象"）。
`expected_gain + no_path = 组内类数`（测试断言守恒）：

| 组 | 类数 | 预计可覆盖 | 无取法（需人工） |
|---|---|---|---|
| 粒子/DEM | 6 | **6** | — |
| 几何/MDL | 5 | 3 | `ISVertex`/`WrappingParam` |
| 混合物/燃烧 | 4 | 3 | `CondReactionIncompSpecies` |
| CoSim | 3 | 3 | — |
| 条件/向导 | 3 | 3 | — |
| 其他 | 3 | 3 | — |
| 材料/物性 | 1 | 0 | `PropDataBase` |
| **合计** | 25 | **21** | 4 |

排期含义：补"粒子/DEM"语料收益最大（6 类），且 4 个无取法的类别指望"补语料"能解决。

### R54-3 复验提醒接进回归入口

`run_all_tests.reopen_notice()`：跑一次 `tools/sweep_reopen_check.py --json`，
**健康时返回空串（安静）**，建议重开时把硬理由打出来（测试用假 subprocess 验"会出声"）。
运行入口现在是：契约门 → 复验提醒 → 各测试模块。

### 回归

全量回归 **1604 passed / 4 skipped / 0 failed**（732.30 s；R53 收口同口径 1592/4/0，
增量 +12 = 本轮新模块 12 项。耗时比往常长约 2.5 min —— 自测与复验提醒各起子进程，',
属预期）。新增测试 1 个模块 `test_r54_evidence.py`（12 项：
自测总账四项 / 收益守恒与证据化 / 提醒静默与出声）。

### 证据

`tools/api_contract_check.py`（`--self-test` 8/8）、`schemas/unswept_account.json`
（`needs_corpus_plan`）、`run_all_tests.py`（`reopen_notice()`）。
...
* 门、文档、面板、复验四件事都已闭环，但**收口结论本身**还没有一份"给人读的总结"
  （散在 ROUNDS 的 R45–R54 十条记录与审计 §59–§67 九节里）；
* `needs_corpus_plan` 的收益估计是**静态**的（按取法可达性），没有"补语料要花多少工"；
* `--self-test` 只覆盖 8 项门，**没有覆盖"文档/面板与产品面同源"**（那两条靠各轮测试）。

### 条目

| # | 条目 | 做法 | 验收句 | 人日 |
|---|---|---|---|---|
| **R55-1** | **收口总结一页** | `docs/HOST_API_COVERAGE.md`：口径 / 数字 / 终态分布 / 复验窗口 / 适用边界（一页读完） | 一处可读 + 与证据对账测试 | 0.5 |
| **R55-2** | **补语料成本粗估** | 收益估计旁边补"要什么算例"（材料库/CoSim/粒子/映射各自的来源与量级） | 每组带成本档（小/中/大）+ 测试 | 0.25 |
| **R55-3** | **同源面进自测** | `--self-test` 增加两项：文档块 == report、面板文本含四段 | 自测 10/10 | 0.25 |

### 明确不做

* 不重跑普查；不动 `host_absent`/验身判据；不为"补语料"去造语料。
