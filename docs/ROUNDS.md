...
* 普查只覆盖 **17/199 类**：因为只有这些类能在单会话里取到实例 —— 而 `Cond*` 里
  **有 89 个 `CreateCond*` 创建器**（收割工具早就用过），批量建实例即可把覆盖率提到 ~100 类；
* `empty_objects` 的 8 个类与 R41/R42 的 NYI 完全重合 —— 值得在面板/文档侧显式提示
  （"该设置需要先跑 MDL/材料流程"）；
* `host_absent` 已入目录，但**面板写回路径**还不知道（枚举白名单只认实测键）。

### 条目

| # | 条目 | 做法 | 验收句 | 人日 |
|---|---|---|---|---|
| **R44-1** | **普查扩面到 `Cond*`** | 用 `CreateCond*`（89 个创建器）批量建条件实例后普查 | 覆盖类数 ≥60；新未实现成员入册 | 0.5 |
| **R44-2** | **空对象提示** | 面板/文档侧对 `empty_objects` 的类给出"需先跑 XX 流程" | 提示可查（工具或文档一处） | 0.25 |
