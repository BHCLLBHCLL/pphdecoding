...
### 64.1 判据的另一面：误杀（R50-1）

R49 只量了误放（0/120）。判据是"独有成员解析率 ≥ 半数"，真正可能**误杀真对象**的是
独有成员少的类。三条证据：

| 指标 | 值 |
|---|---|
| 自类通过（**全部**已取得类，不抽样） | **108/108**，`false_kill = 0` |
| 跨类误放（抽样 40 类 × 3 别家） | **0/120（0.0%）** |
| 被否样本离阈值多远 | 去重 **6** 条，近阈（`|2r − s| ≤ 1`）**0** 条 |
| 阈值边界表（样本 1..5 × 解析 0..s） | `2r ≥ s` 放行；`2r = s − 1` 拒绝（离线确定性） |

**判据建议**：维持"半数"不变 —— 108/108 与 0/120 说明它在真数据上既不误杀也不误放；
被否的 6 条全都远在阈值之下（不是"差一点翻案"的脆判决）。

### 64.2 证据可复验串（R50-2）

每轮普查带 `--evidence-run <轮次>`，写进证据并在提取期灌进目录条目：

`R50 | 2026-09-23T20:52:01 | log=_p12u_gate\r50\name_verdicts.json |
projects=box.pph,exB01-1_intake_manifold.pph,exA26-1_ldc.pph,exA16-2.pph,exA25-1.pph,exA18-4.pph`

`host_absent`（27 条）与 `recipe_unreliable`（4 类）逐条带 `evidence_run` ——
任取一条都能追到"哪一轮、哪个日志、哪些工程、什么时候"，复验不必翻全量日志。
测试断言：**每一条**都带，且串里含轮次与 `log=`。

### 64.3 普查收口声明（R50-3）

**声明**：域「宿主 VB API 成员可用性普查」至此**收口**，判据三条同时成立：

1. **剩余类全部有终态**：29/29（`schemas/unswept_account.json`，六种终态，零"未归因"）；
2. **边际收益 < 1 类/轮**的趋势已现：R47 +3（词表/名字池）→ R48 +3（命名片段）→
   R49 +3（先全选）→ R50 +0（本轮只做误杀与追溯）—— 机制驱动的增益已被吃尽，
   剩下 25/29 是 `needs-corpus`（缺**语料**不是缺机制）；
3. **覆盖率锁死**：`test_r50_evidence.py` 把下限钉在 **155 类**（只许升不许降）。

**复验窗口**：出现以下任一条即重开 —— ① 新增语料（材料库/CoSim/粒子/映射算例）；
...
## 68. R54 更新（2026-09-15）—— 契约门自测总账 + 缺语料收益估计 + 复验提醒落点

### 68.1 每项门都有反例（R54-1）

`python tools/api_contract_check.py --self-test`：8 项各喂一个合成反例（假参数 / 缺口无终态 /
仓内引用未实现成员 / 计数对不上 / 覆盖率 0.5 / 语料漏项 / 三态判据坏 / 覆盖率 3），
**必须全部 FAIL**（当前 8/8）。为可注入性给 4 项加路径参数，另 3 项用 `_Patch` 临时替换
模块级名字（测试断言跑完还原）。

> 自测首轮 4 项"放行"：① 4 个 lambda 捕获同一个变量 `p`（**晚绑定**）；② 三项检查没有注入点。
> 教训：**门必须能拿假证据自证** —— 否则"门是好的"永远只是假设。

### 68.2 缺语料收益估计（R54-2）

`needs_corpus_plan`：判据 = 有已知取法（候选非空，或手册给了配方）。守恒式
`expected_gain + no_path = 组内类数`。合计 25 类 → **预计 21 类可覆盖**，4 类无取法
（`ISVertex`/`WrappingParam`/`CondReactionIncompSpecies`/`PropDataBase`）。
排期含义：粒子/DEM 组收益最大（6 类）。

### 68.3 复验提醒（R54-3）

`run_all_tests.reopen_notice()`：健康时静默，建议重开时打硬理由；入口顺序变为
**契约门 → 复验提醒 → 各测试模块**。
